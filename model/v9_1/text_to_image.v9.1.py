"""V9.1 — V8 baseline with stronger image-text supervision.

V9.1 keeps the fast V8 generator and projection discriminator, while adding
an explicit batch image-text contrastive objective. The goal is to make the
caption materially affect generated content without the expensive V9 FiLM
architecture.

Uses the same dataset: ./data/captions.txt and ./data/image60px.
Starts from scratch in ./model/v9_1.
"""
...
def make_discriminator():
    ...
    conditional_score = layers.Add(name="conditional_score")([realness, compatibility])
...
def gen_step(
    ...
    with tf.GradientTape() as tape:
        text_features = tf.stop_gradient(text_encoder(caption_tokens, training=False))
        noise = tf.random.normal([batch_size, NOISE_DIM])
        fake_images = generator([noise, text_features], training=True)
...
def main():
    ...
    if len(MONITOR_CAPTIONS) < MONITOR_GRID:
        MONITOR_CAPTIONS = (
            MONITOR_CAPTIONS
            + [
                "a body of water",
    ...
    latest_ckpt = tf.train.latest_checkpoint(V91_CHECKPOINT_DIR)

    if latest_ckpt:
        checkpoint.restore(latest_ckpt).expect_partial()
        print(f"Resumed V9.1 checkpoint: {latest_ckpt}")
    else:
        print("Starting V9.1 from scratch with the current captions.txt.")

    print()
    print("V9.1 configuration")
    print("================")
    ...
    print(
        "Generator architecture: stable V8 baseline + image-text contrastive supervision"
    )
    print(f"Mismatch loss weight: " f"{MISMATCH_LOSS_WEIGHT:.2f}")
    print(f"Alignment loss weight: " f"{ALIGNMENT_LOSS_WEIGHT:.2f}")
    print(f"Alignment temperature: " f"{ALIGNMENT_TEMPERATURE:.2f}")
