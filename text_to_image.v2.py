...
BATCH_SIZE = 16
NOISE_DIM = 128
FINE_TUNE_FROM_CHECKPOINT = True
...
def main():
    ...
    if FINE_TUNE_FROM_CHECKPOINT:
        if not latest_ckpt:
            raise FileNotFoundError(
                "FINE_TUNE_FROM_CHECKPOINT=True, but no checkpoint was found in "
                f"{checkpoint_dir!r}. Keep the epoch-300 checkpoint directory "
        ...
        try:
            ...
            print(f"Restored epoch-300 model from: {latest_ckpt}")
            print("Fine-tuning with a lower discriminator learning rate.")
        except Exception as exc:
            raise RuntimeError(
                f"Checkpoint restore failed for fine-tuning: {latest_ckpt}"
            ) from exc
    ...
    print(
        f"Training schedule: epochs {start_epoch + 1}-{total_epochs} | "
        f"gen_lr={float(gen_opt.learning_rate):.1e} | "
        f"disc_lr={float(disc_opt.learning_rate):.1e}"
    )

    # Keep the same fixed noise vectors, but use different captions so we can
    # test whether the generator responds to text rather than only producing
    # a generic landscape.
    monitor_noise = tf.random.normal([MONITOR_GRID, NOISE_DIM], seed=SEED)
    ...
    for local_epoch in range(FINE_TUNE_EPOCHS):
        g_loss_metric.reset_state()
        d_loss_metric.reset_state()
        epoch_start = time.time()

        actual_epoch = start_epoch + local_epoch + 1
        ...
        # Save regular samples and inspect caption-to-image changes.
        if actual_epoch % 10 == 0 or actual_epoch == total_epochs:
    ...
    generator.save("generator_model_60px_finetuned.keras")
    g_ema.save("generator_ema_model_60px_finetuned.keras")
    discriminator.save("discriminator_model_60px_finetuned.keras")
    text_encoder.save("text_encoder_60px_finetuned.keras")

    # Persist tokenizer vocabulary/config needed for inference.
    tokenizer_config = tokenizer.to_json()
    with open("tokenizer_60px_finetuned.json", "w", encoding="utf-8") as f:
